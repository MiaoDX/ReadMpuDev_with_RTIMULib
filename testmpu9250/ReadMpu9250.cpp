...
//class mpu9250Angle
int mpu9250Angle::mpu9250AngleInit(int _bufferSize /*= 200*/, int _compareSizeStill /*= 100*/, int _compareSizeMoving /* = 10*/) {	//确定当前位置时使用 compareSize = 100，运动过程中实时计算取 30 即可
...
void mpu9250Angle::mpu9250AngleBufferUpdate(RTVector3_T<RTFLOAT> newAngle) {
...
inline RTVector3_T<RTFLOAT> mpu9250Angle::CalculateAvg(list<RTVector3_T<RTFLOAT>> &callist)
{
...
float mpu9250Angle::mpu9250AnglePresentUpdate(bool moving/* = false*/) {	//moving 指示是否是正在运行
...
//bool mpu9250Angle::mpu9250AngleReachTarget(RTVector3_T<RTFLOAT> targetAngle, float precision/* = 0.1*/) {
	////先更新当前值
	//mpu9250AnglePresentUpdate();
//
...
RTVector3_T<RTFLOAT> mpu9250Angle::getAnglePresent() {
	return presentAngle;
}
	...
	while (true) {
		...
		while (imu->IMURead()) {
			...
			if ((now - displayTimer) > 10000) {
				...
				mpuAngle.mpu9250AngleBufferUpdate(RTVector3_T<RTFLOAT>(imuData.fusionPose.x() * RTMATH_RAD_TO_DEGREE, imuData.fusionPose.y() * RTMATH_RAD_TO_DEGREE, imuData.fusionPose.z() * RTMATH_RAD_TO_DEGREE));
